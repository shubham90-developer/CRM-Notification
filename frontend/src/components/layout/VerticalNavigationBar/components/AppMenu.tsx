...
const AppMenu = ({ menuItems }: AppMenuProps) => {
  ...
  const filteredMenuItems = menuItems.filter((item) => {
    if (!(item as any).roles) return true // no roles restriction = show to all
  ...
  const activeMenu = useCallback(() => {
    ...
    if (matchingMenuItem) {
      ...
      if (activeMt) {
        const newItems = [activeMt.key, ...findAllParent(filteredMenuItems, activeMt)]
        setActiveMenuItems((prev) => {
          const isSame = prev.length === newItems.length && prev.every((v, i) => v === newItems[i])
